...
cd /var/log/apache2
tar -cf /tmp/${krishna}-httpd-logs-${timestamp}.tar *.log

# copy logs to s3 bucket
if [[ -f /tmp/${krishna}-httpd-logs-${timestamp}.tar ]]; then
	#statements
	aws s3 cp /tmp/${krishna}-httpd-logs-${timestamp}.tar s3://${s3_bucket}/${name}-httpd-logs-${timestamp}.tar

fi
docroot="/var/www/html"
#check if inventory files excits 
...
size=$(du -h /temp/${name}-httpd-logs-${timestamp}.tar | awk '{print $1}')
  echo -e "httpd-logs\t-\t${timestamp}\t-\ttar\t-t${size}" >> ${docroot}/inventory.html 
fi 
